...
export async function POST(request: NextRequest) {
  ...
  if (!password) {
    return NextResponse.json(
      { error: "What's Baking admin is not configured" },
      { status: 503 }
    );
  }
  ...
  if (!title || !summary || !bodyText) {
    return NextResponse.json(
      { error: "Title, summary, and post body are required" },
      { status: 400 }
    );
  }
  ...
  const imageAlt = clean(form.get("imageAlt"), 200) || title;
  ...
  if (file instanceof File && file.size > 0) {
    if (file.size > MAX_IMAGE_BYTES) {
      return NextResponse.json(
        { error: "Image is too large. Maximum is 5 MB." },
        { status: 400 }
      );
    }
    ...
    const imageName = `whats-baking/${id}.${ext}`;
  ...
  const withoutDuplicate = posts.filter(
